    ...
    def process_all(self, environment: str):
        ...
        for garment in files:
            try:
                ...
                garment_number, garment_type = extract_metadata_from_filename(garment.name)
                logger.info(f"Processing {garment.name} - Number: {garment_number}, Type: {garment_type}")

                # Validate metadata before processing
                if not garment_number or not garment_type:
                    logger.error(f"Skipping {garment.name}: Missing metadata (number={garment_number}, type={garment_type})")
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--single", type=str, help="Process only a single image file (path to image)")
    parser.add_argument("--test", action="store_true", help="Test mode: process only the first image found")
    parser.add_argument("--number", type=str, help="Garment number (optional, overrides auto-detection)")
    parser.add_argument("--type", type=str, help="Garment type (optional, overrides auto-detection)")
    ...
    if args.single:
        ...
        if args.number and args.type:
            ...
            logger.info(f"Using provided metadata - Number: {garment_number}, Type: {garment_type}")
        else:
            garment_number, garment_type = extract_metadata_from_filename(single_file.name)
            logger.info(f"Extracted from filename - Number: {garment_number}, Type: {garment_type}")

        logger.info(f"Processing single file: {single_file.name}")

        # Validate metadata
        if not garment_number or not garment_type:
            logger.error(f"Missing required metadata: number={garment_number}, type={garment_type}")
            logger.error("Please provide both --number and --type, or use a filename like: 42_pantalon_20251229.HEIC")
            exit(1)
        ...
        if args.number and args.type:
            ...
            garment_number, garment_type = extract_metadata_from_filename(test_file.name)
            logger.info(f"Extracted from filename - Number: {garment_number}, Type: {garment_type}")
